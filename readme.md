...
## 2 Requirements
`python>=3.9`, `torch==2.3.0`, `cityflow`, `pandas`, `numpy`

Recommended installation:
```shell
pip install torch==2.3.0 cityflow pandas numpy
```

[`cityflow`](https://github.com/cityflow-project/CityFlow.git) needs a linux environment, and we run the code on Manjaro Linux.

The main learning agents now run on native PyTorch.
Current PyTorch migration covers `EfficientMPLight`, `AdvancedMPLight`, `EfficientPressLight`,
`AdvancedDQN`, `Attend`, `EfficientColight`, `AdvancedColight`, and `PPOColight`.
These agents save checkpoints as `.pt` files in `PATH_TO_MODEL`.
