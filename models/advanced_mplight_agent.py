...
class AdvancedMPLightNet(nn.Module):
    def __init__(self, num_actions, d_dense, phase_list, lane_order, dic_index):
        ...
        self.num_vec_mapping_part = nn.Linear(1, 4)
        self.lane_embedding = nn.Linear(12, 24)
        self.relation_embedding = nn.Embedding(2, 4)
        self.lane_conv = nn.Conv2d(48, d_dense, kernel_size=1)
    ...
    def forward(self, inputs):
        ...
        for i, movement in enumerate(self.lane_order):
            ...
            vec_1 = torch.sigmoid(self.num_vec_mapping(feat2[:, idx:idx + 1]))
            vec_2 = torch.sigmoid(self.num_vec_mapping_part(feat3[:, idx:idx + 1]))
            phase_vec = phase_embeddings[:, i, :]
            lane_features[movement] = torch.cat([vec_1, phase_vec, vec_2], dim=-1)
        ...
        feature_map = feature_map.reshape(feat2.shape[0], num_phase, num_phase - 1, 48).permute(0, 3, 1, 2)
