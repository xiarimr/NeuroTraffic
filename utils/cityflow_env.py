...
class Intersection:
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict, path_to_log, lanes_length_dict):
        ...
        self.main_road_approaches = dic_traffic_env_conf.get("REWARD_MAIN_ROAD_APPROACHES", ["W", "E"])

        # generate all lanes
        self.list_entering_lanes = []
        self.dic_approach_to_entering_lanes = {}
        for (approach, lane_number) in zip(self.list_approachs, dic_traffic_env_conf["NUM_LANES"]):
            entering_lanes = [self.dic_entering_approach_to_edge[approach] + "_" + str(i) for i in range(lane_number)]
            self.dic_approach_to_entering_lanes[approach] = entering_lanes
            self.list_entering_lanes += entering_lanes
        self.list_exiting_lanes = []
        for (approach, lane_number) in zip(self.list_approachs, dic_traffic_env_conf["NUM_LANES"]):
            self.list_exiting_lanes += [self.dic_exiting_approach_to_edge[approach] + "_" + str(i) for i in
                                        range(lane_number)]
        self.main_road_entering_lanes = []
        for approach in self.main_road_approaches:
            self.main_road_entering_lanes.extend(self.dic_approach_to_entering_lanes.get(approach, []))
        ...
        self.dic_lane_vehicle_left_count_current_step = {}
    ...
    def update_current_measurements(self, simulator_state):
        ...
        self.dic_lane_vehicle_left_count_current_step = self._get_lane_vehicle_left_counts()
    ...
    def _get_lane_vehicle_left_counts(self):
        lane_vehicle_left_count = {}
        for lane in self.list_entering_lanes:
            previous_vehicles = set(self.dic_lane_vehicle_previous_step_in.get(lane, []))
            current_vehicles = set(self.dic_lane_vehicle_current_step_in.get(lane, []))
            lane_vehicle_left_count[lane] = float(len(previous_vehicles - current_vehicles))
        return lane_vehicle_left_count
    ...
    def _update_feature(self):
        ...
        dic_feature["pressure_total"] = float(np.absolute(np.sum(dic_feature["pressure"])))
        dic_feature["adjacency_matrix"] = self._get_adjacency_row()
        dic_feature["queue_length"] = float(np.sum(dic_feature["lane_num_waiting_vehicle_in"]))
        dic_feature["delay"] = float(np.sum(dic_feature["lane_num_waiting_vehicle_in"]) +
                                     np.sum(dic_feature["lane_num_waiting_vehicle_out"]))
        dic_feature["throughput"] = float(sum(self.dic_lane_vehicle_left_count_current_step.values()))
        dic_feature["phase_switch"] = float(self.flicker)
        dic_feature["main_road_queue_length"] = float(sum(
            self.dic_lane_waiting_vehicle_count_current_step.get(lane, 0)
            for lane in self.main_road_entering_lanes
        ))
        dic_feature["main_road_throughput"] = float(sum(
            self.dic_lane_vehicle_left_count_current_step.get(lane, 0.0)
            for lane in self.main_road_entering_lanes
        ))
